...
async def admin_panel(client, message):
    await safe_reply(message, owner_panel_text(), reply_markup=owner_panel_keyboard(), disable_web_page_preview=True)
...
async def owner_callbacks(client, query):
    if query.from_user.id != OWNER.ID:
        return await safe_callback_answer(query, "Owner only.", show_alert=True)
    action = query.data.split(":", 1)[1]
    if action in {"panel", "stats"}:
        await safe_callback_answer(query, "Panel refreshed ✅")
        return await safe_edit_message(query.message, owner_panel_text(), reply_markup=owner_panel_keyboard(), disable_web_page_preview=True)
    if action == "fsubs":
        await safe_callback_answer(query, "Force-sub list opened 🔐")
        return await safe_edit_message(query.message, fsub_text(), reply_markup=owner_back_keyboard(), disable_web_page_preview=True)
    if action == "broadcast":
        await safe_callback_answer(query, "Broadcast help opened 📣")
        return await safe_edit_message(query.message,
    ...
    await safe_callback_answer(query, "Commands opened 📚")
...
async def add_fsub_cmd(client, message):
    if len(message.command) < 2:
        return await safe_reply(message, "Usage: `/addfsub -1001234567890 https://t.me/example`", reply_markup=owner_back_keyboard())
    try:
        chat_id = int(message.command[1])
    except ValueError:
        return await safe_reply(message, "Chat ID must be numeric.", reply_markup=owner_back_keyboard())
    ...
    await safe_reply(message, f"Force-sub chat added ✅\nID: `{chat_id}`", reply_markup=owner_back_keyboard())

@Client.on_message(filters.private & owner_filter & filters.command("delfsub"))
async def del_fsub_cmd(client, message):
    if len(message.command) < 2:
        return await safe_reply(message, "Usage: `/delfsub -1001234567890`", reply_markup=owner_back_keyboard())
    database.remove_fsub(int(message.command[1]))
    await safe_reply(message, "Force-sub chat removed ✅", reply_markup=owner_back_keyboard())

@Client.on_message(filters.private & owner_filter & filters.command("fsubs"))
async def list_fsub_cmd(client, message):
    await safe_reply(message, fsub_text(), reply_markup=owner_back_keyboard(), disable_web_page_preview=True)
...
async def broadcast_cmd(client, message):
    ...
    if not targets:
        return await safe_reply(message, "No broadcast targets found yet.", reply_markup=owner_back_keyboard())

    status_message = await safe_reply(message, f"Broadcast started to `{len(targets)}` targets using forward method...")
    ...
    if status_message:
        await safe_edit_message(status_message, result_text, reply_markup=owner_panel_keyboard())
    else:
        await safe_reply(message, result_text, reply_markup=owner_panel_keyboard())
