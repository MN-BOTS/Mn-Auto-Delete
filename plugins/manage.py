...
import database
from utils import parse_time, human_time, is_chat_admin, settings_keyboard, settings_text
...
async def delete_on_cmd(client, message):
    if not await can_manage(client, message):
        return await deny(message)
    settings = database.get_chat_settings(message.chat.id)
    delay = int(settings.get("delete_delay") or 0) or 60
...
@Client.on_callback_query(filters.regex(r"^(settings|set|delay):"))
...
async def chat_help(client, message):
    await message.reply_text(
        "**MN Auto Delete Help**\n\n"
        "`/settings` - open rich settings panel\n"
        "`/setdelete <time>` - set timer: 10s, 5m, 1h, 1d\n"
        "`/deleteon` - enable with saved/default time\n"
        "`/deleteoff` - disable deleting\n\n"
