...
async def deny(message):
    await safe_reply(message, "**Only the group/channel owner or admins can manage this bot here.**")
...
async def set_delete_cmd(client, message):
    ...
    if len(message.command) < 2:
        return await safe_reply(message, "Usage: `/setdelete 30s` or `/setdelete off`", quote=True)
    try:
        seconds = parse_time(message.command[1])
    except ValueError as err:
        return await safe_reply(message, f"Invalid time: {err}\nExamples: `10s`, `5m`, `1h`, `off` (1 hour max)", quote=True)
...
async def settings_callbacks(client, query):
    ...
    if query.from_user.id != OWNER.ID and not await is_chat_admin(client, chat_id, query.from_user.id):
        return await safe_callback_answer(query, "Admins only.", show_alert=True)

    try:
        chat = await client.get_chat(chat_id)
    except Exception:
        return await safe_callback_answer(query, "I cannot access that chat anymore.", show_alert=True)
    if action == "set":
        enabled = parts[2] == "on"
        ...
        await safe_callback_answer(query, f"Delay set to {human_time(seconds)} ✅")
    elif action == "chathelp":
        await safe_callback_answer(query, "Help opened 📘")
        return await safe_edit_message(query.message, chat_help_text(), reply_markup=settings_keyboard(chat_id, database.get_chat_settings(chat_id).get("enabled")))
    else:
        await safe_callback_answer(query, "Refreshed 🔄")

    settings = database.get_chat_settings(chat_id)
    await safe_edit_message(query.message, settings_text(chat, settings), reply_markup=settings_keyboard(chat_id, settings.get("enabled")))
